...
dm_dbcsr distribute(std::vector<double>& loc_matrix, std::vector<int>& row_blk_sizes, std::vector<int>& col_blk_sizes,
  ...
  dbcsr::create_new(dmat, "this is some matrix", dist, 'N',
  ...
  for(int i = 0; i < (int)row_blk_sizes.size(); i++){
    ...
    for(int j = 0; j < (int)col_blk_sizes.size(); j++){
      ...
      int blk_proc = -1;
      dbcsr::get_stored_coordinates(dmat, i, j, &blk_proc);
      if(blk_proc == mpi_rank){
        ...
        dbcsr::put_block(dmat, i, j, block.data(), block.size());
      }
      joff += jdim;
    }
    ioff += idim;
  ...
  for(int i = 0; i < (int)row_blk_sizes.size(); i++)
  {
      ...
      for(int j = 0; j < (int)col_blk_sizes.size(); j++)
      {
          ...
          if(blk_proc == mpi_rank)
          {
              ...
              double* bptr = &block[0];
              bool foundit = false;
              dbcsr::get_block(dmat,i,j,bptr,foundit,idim,jdim);
              if (foundit){
                for(int cc=0;cc<jdim;cc++){
                  for(int rr=0;rr<idim;rr++){
...
void gershgorin_dist(dm_dbcsr& dist_mat, int ldim, double& eps0, double& epsn,
  ...
  dbcsr::gershgorin_estimate(dist_mat,row_blk_sizes.data(),row_blk_sizes.size(),ldim,sums.data(),diags.data());
...
int main(int argc, char* argv[])
{
    ...
    dbcsr::distribution_new(dist, group,row_dist.data(), row_dist.size(),
    ...
    dbcsr::multiply('N', 'N', 1.0, matrix_a, matrix_b, 0.0, matrix_c);
    ...
    dbcsr::copy(matrix_b,matrix_a);
    dbcsr::add(matrix_a, matrix_b, 1.e0, -1.e0);
    ...
    // trace ab
    dtr = 0.e0;
    dbcsr::trace_ab(matrix_a,matrix_b,dtr);
    ltr = 0.e0;
    ...
    dbcsr::set(matrix_a,0.e0);
    dbcsr::set_diag(matrix_a,diags.data(),nrow_tot);
    ...
    if (mpi_rank == 0) printf("dbcsr::print(matrix_c):\n");
    fflush(stdout);
    ...
    dbcsr::finalize_lib_silent(group);
