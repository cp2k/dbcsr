...
def main():
    """
    Generate a performance test of libcusmm in the form of a CUDA file, using libcusmm_timer_multiply.template as a template
    """
    parser = OptionParser()
    parser.add_option("-f", "--base_folder", metavar="DBCSRHOME", default="",
    ...
    parser.add_option("-n", "--nsamples", default=1000,
    ...
    with open(file_generate, 'w') as f:
        f.write(test)
    print("Wrote", len(kernels_to_print), "test kernels to", file_generate)
