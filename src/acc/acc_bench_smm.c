...
static void parse_params(int argc, char *argv[], FILE **file, const char **snr, const char **sss, const char **ssm,
  ...
  --argc;
  if (NULL == *file)
    *file = (1 <= argc ? fopen(argv[1], "r") : NULL);
  if (NULL == *file)
    for (i = 0; i < argc; ++i)
      args[i] = argv[i + 1];
  else {
...
#endif
        for (i = 0; i < na; ++i)
          INIT_MAT(ELEM_TYPE, i /*seed*/ + 42, &amat_hst[i * mk], m, k, 1.0 / (nr * na));
#if defined(_OPENMP)
#pragma omp for
#endif
        for (i = 0; i < nb; ++i) {
    ...
    for (r = 0; r < warmup / 2; ++r) {
      CHECK(libsmm_acc_transpose(trans_dev, 0 /*offset*/, nb, bmat_dev, DBCSR_TYPE(ELEM_TYPE), k, n, MAX_KERNEL_DIM,
                                 stream),
            &result);
      CHECK(libsmm_acc_transpose(trans_dev, 0 /*offset*/, nb, bmat_dev, DBCSR_TYPE(ELEM_TYPE), n, k, MAX_KERNEL_DIM,
                                 stream),
            &result);
    }
#if defined(USE_LIBXSMM)
    ...
    for (r = 0; r < warmup; ++r) {
      CHECK(libsmm_acc_process(stack_hst, stack_dev, stack_size, 3 /*nparams*/, DBCSR_TYPE(ELEM_TYPE), amat_dev,
                               bmat_dev, cmat_dev, m, n, k, MAX_KERNEL_DIM, 1 /*homogeneous*/, stream, stream),
            &result);
    }
    CHECK(c_dbcsr_acc_memset_zero(cmat_dev, 0 /*offset*/, sizeof(ELEM_TYPE) * mn * nc, stream), &result);
#if defined(USE_LIBXSMM)
...
#if defined(VALIDATE)
    {
      ...
      if (NULL != gold_hst && NULL != amat_hst && NULL != bmat_hst && NULL != stack_hst) {
        ...
        for (r = 0; r < warmup; ++r) {
          ...
          (LIBXSMM_DATATYPE(ELEM_TYPE), LIBXSMM_DATATYPE(ELEM_TYPE), &transa, &transb, m, n, k, &alpha, amat_hst,
           &m /*lda*/, bmat_hst, &k /*ldb*/, &beta, gold_hst, &m /*ldc*/, 1 /*index_base*/, sizeof(int) * 3,
        ...
        if (EXIT_SUCCESS == result) {
          ...
          if (EXIT_SUCCESS == result) {
            ...
            if (EXIT_SUCCESS == result) {
              ...
              if (maxerror < relerror && NULL != file)
                maxerror = relerror;
              if (0 < relerror) {
