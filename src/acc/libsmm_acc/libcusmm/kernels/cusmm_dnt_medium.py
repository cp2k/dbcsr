...
class Kernel_dnt_medium(Kernel):
        ...
        for minblocks_ in range(1, 28) if minblocks is None else [minblocks]:
            ...
            for grouping_ in range(1, 32 + 1, 1) if grouping is None else [grouping]:
                ...
                for tm in (range(1,
                    ...
                    for tn in (range(1,
                                     min(12, n) + 1)
                               if tile_n is None else [tile_n]):  # heuristic: the optimal tile_m is never above 12
                        ...
                        max_concurrent_work = max(grouping_, m * k, k * n, m * n, cmax * rmax)
    ...
    def baseline(m, n, k, gpu, autotuning):
        ...
        while True:
            ...
            if len(Kernel_dnt_medium.promising_parameters(m, n, k, gpu, autotuning, **base)) > 0:
                break
            else:
                grp -= 1
                if grp == 0:
                    base = Kernel_dnt_medium.promising_parameters(m, n, k, gpu, autotuning)[0]
                    base.update(dict([("w", np.NaN), ("v", np.NaN)]))
                    break

        base.update(dict([("m", m), ("n", n), ("k", k), ("algorithm", "medium"), ("perf", 0), ("source", "predicted")]))
        return base
