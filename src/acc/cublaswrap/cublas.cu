				 ...
				 double alpha, double beta, cudaStream_t *stream)
{
  ...
  for (int ii = 0; ii < stack_size; ii++) {
    ...
    int m = stack_params[ ps_width * ii ];
    int n = stack_params[ ps_width * ii + 1];
    int k = stack_params[ ps_width * ii + 2];
    ...
    cublasStatus_t stat = cublasDgemm(*handle, cTransa, cTransb, m, n, k, &alpha, a_mat, m, b_mat, k, &beta, c_mat, m);
