...
#if LIBXSMM_VERSION4(1, 16, 1, 808) <= LIBXSMM_VERSION_NUMBER &&                                                       \
    LIBXSMM_VERSION4(1, 17, 0, 2042) > LIBXSMM_VERSION_NUMBER
...
#define OPENCL_LIBSMM_TYPESIZE(TYPEID)                                                                                 \
  (dbcsr_type_real_8 == (TYPEID) ? ((int)sizeof(double))                                                               \
                                 : (dbcsr_type_real_4 == (TYPEID) ? ((int)sizeof(float)) : 0 /*unknown*/))
  ...
  for (i = 0; i < m; ++i) {
    ...
    for (j = 0; j < n; ++j) {
      ...
      case dbcsr_type_real_8:
        fprintf(ostream, "%.2f ", ((double *)mat)[i * n + j]);
        break;
      case dbcsr_type_real_4:
        fprintf(ostream, "%.2f ", ((float *)mat)[i * n + j]);
        break;
      default:
        fprintf(ostream, "? ");
      }
...
int opencl_libsmm_write_trans_params(FILE *stream, int only_key, const opencl_libsmm_transkey_t *key,
  ...
  if (NULL != stream) {
    ...
    if (NULL != key || 0 == only_key)
      result += fprintf(stream, "%c", NULL == begin ? '{' : *begin);
    if (NULL != config) {
      if (NULL != key) {
    ...
    if (NULL != key || 0 == only_key)
      result += fprintf(stream, "%c", NULL == close ? '}' : *close);
  } else
    result = -1;
  assert(0 < result);
  return result;
    ...
    if (NULL != config) {
      ...
      if (0 == only_key)
        result += fprintf(stream,
                          ...
                          "%c%i%c%i%c%i%c%i%c%i"
                          "%c%i%c%i%c%i%c%i%c%i",
                          config->bs, d, config->bm, d, config->bn, d, config->bk, d, config->ws, d, config->wg, d,
                          config->lu, d, config->nz, d, config->al, d, config->tb, d, config->tc, d, config->ap, d,
                          config->aa, d, config->ab, d, config->ac);
    } else {
      if (NULL != key) {
        result += fprintf(stream,
                          "t%c"
  ...
  if (max_consumed == consumed) {
    if (NULL != perfest) {
      ...
      case dbcsr_type_real_8: {
        ...
        if (perfest->gf_ai_dratio_max < ratio)
          perfest->gf_ai_dratio_max = ratio;
        ++perfest->dcount;
      } break;
      case dbcsr_type_real_4: {
        ...
        if (perfest->gf_ai_sratio_max < ratio)
          perfest->gf_ai_sratio_max = ratio;
        ++perfest->scount;
      } break;
      default:
        result = EXIT_FAILURE;
  ...
  if (1 == LIBXSMM_ATOMIC_ADD_FETCH(&opencl_libsmm_initialized, 1, LIBXSMM_ATOMIC_RELAXED)) {
    ...
    if (EXIT_SUCCESS == result) {
      ...
      if (NULL == env_params || '0' != *env_params) {
        char buffer[ACC_OPENCL_BUFFERSIZE], control = '0';
        ...
        if (NULL != env_params && '\0' != *env_params) { /* filename */
          ...
          if (NULL != file) {
            ...
            if (NULL != fgets(buffer, ACC_OPENCL_BUFFERSIZE, file)) {
              ...
              char *const device = (NULL != libxsmm_stristr(buffer, "device") ? devname : NULL);
        ...
        if (EXIT_SUCCESS == result && '1' != control) {
          ...
          do {
            ...
            if (NULL != next && next < (line + ACC_OPENCL_BUFFERSIZE)) {
              ...
              buffer[len] = '\0';
              if (EXIT_SUCCESS == opencl_libsmm_read_smm_params(buffer, &key, &config, &perfest,
                                                                NULL /*device*/)) /* read params from embedded params */
              {
                if (NULL == OPENCL_LIBSMM_REGISTER(&key, sizeof(key), sizeof(config), &config)) {
                  ACC_OPENCL_DEBUG_FPRINTF(stderr, "ERROR ACC/OpenCL: libxsmm_xregister failed!\n");
                  result = EXIT_FAILURE;
        ...
        if ('2' == control) {
          if (/* try reading OPENCL_LIBSMM_SMM_PARAMS-value as kernel parameters */
              EXIT_SUCCESS ==
              opencl_libsmm_read_smm_params(env_params, &key, &config, NULL /*perfest*/, NULL /*device*/)) {
            key.devuid = 0;
            if (NULL == OPENCL_LIBSMM_REGISTER(&key, sizeof(key), sizeof(config), &config)) {
              result = EXIT_FAILURE;
            }
...
#endif
              for (i = 0; i < na; ++i)
                INIT_MAT(float, i + 42, &a[i * mk], m, k, 1.0 / (nc * na));
#if defined(_OPENMP)
#pragma omp for
#endif
              for (i = 0; i < nb; ++i)
                INIT_MAT(float, i + 24, &b[i * kn], k, n, 1.0 / (nc * nb));
            }
            memset(c, 0, sizeof(float) * nc * mn);
            start = libxsmm_timer_tick();
            for (i = 0; i < nrepeat; ++i) {
              OPENCL_LIBSMM_USEOMP(libxsmm_gemm_batch)
              (LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, &notrans, &notrans, m, n, k, &alpha, a, &m /*lda*/, b,
               &k /*ldb*/, &beta, c, &m /*ldc*/, 1 /*index_base*/, sizeof(int) * 3, s + 0, s + 1, s + 2, stack_size);
            }
            opencl_libsmm_shst =
                1E-9 * ((size_t)2 * m * n * k * stack_size * nrepeat) /
...
#endif
              for (i = 0; i < na; ++i)
                INIT_MAT(double, i + 42, &a[i * mk], m, k, 1.0 / (nc * na));
#if defined(_OPENMP)
#pragma omp for
#endif
              for (i = 0; i < nb; ++i)
                INIT_MAT(double, i + 24, &b[i * kn], k, n, 1.0 / (nc * nb));
            }
            memset(c, 0, sizeof(double) * nc * mn);
            start = libxsmm_timer_tick();
            for (i = 0; i < nrepeat; ++i) {
              OPENCL_LIBSMM_USEOMP(libxsmm_gemm_batch)
              (LIBXSMM_DATATYPE_F64, LIBXSMM_DATATYPE_F64, &notrans, &notrans, m, n, k, &alpha, a, &m /*lda*/, b,
               &k /*ldb*/, &beta, c, &m /*ldc*/, 1 /*index_base*/, sizeof(int) * 3, s + 0, s + 1, s + 2, stack_size);
            }
            opencl_libsmm_dhst =
                1E-9 * ((size_t)2 * m * n * k * stack_size * nrepeat) /
...
int libsmm_acc_finalize(void) {
  ...
  if (0 == LIBXSMM_ATOMIC_SUB_FETCH(&opencl_libsmm_initialized, 1, LIBXSMM_ATOMIC_RELAXED)) {
    ...
    for (; NULL != regentry; regentry = libxsmm_get_registry_next(regentry, &regkey)) {
      ...
      if (NULL == kernel)
        kernel = ((const opencl_libsmm_smm_t *)regentry)->kernel[1];
      if (NULL != kernel) {
        if (3 == c_dbcsr_acc_opencl_config.verbosity) {
          char fname[ACC_OPENCL_MAXSTRLEN];
          ACC_OPENCL_CHECK(clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, ACC_OPENCL_BUFFERSIZE, fname, NULL),
                           "retrieve function name", result);
          if (EXIT_SUCCESS == result) {
            if (NULL != strstr(fname, OPENCL_LIBSMM_KERNELNAME_TRANS)) { /* trans-kernel */
              ...
              if (0 < entry->nexec) {
                opencl_libsmm_trans_t dummy;
                const int size = (int)MIN(sizeof(entry->size) / sizeof(*entry->size), entry->nexec);
                int blocks;
                OPENCL_LIBSMM_ISORT(entry->size, size);
                blocks = entry->size[size >> 1];
                if (0 == (1 & size))
                  blocks = (blocks + entry->size[(size >> 1) - 1]) >> 1;
                LIBXSMM_STDIO_ACQUIRE();
                fprintf(stderr, "INFO ACC/OpenCL: TRANS-kernel ");
                opencl_libsmm_write_trans_params(stderr, 1 /*only_key*/, desc, NULL /*config*/, NULL /*delim*/,
                                                 NULL /*begin*/, NULL /*close*/);
                fprintf(stderr, "=");
                opencl_libsmm_write_trans_params(stderr, 1 /*only_key*/, desc, &dummy, NULL /*delim*/, NULL /*begin*/,
                                                 NULL /*close*/);
                fprintf(stderr, " ss=%i geo=%.1f GB/s\n", blocks, exp(entry->membw_sumlog / entry->nexec));
              ...
              if (0 < entry->nexec) {
                ...
                const double est =
                    (dbcsr_type_real_8 == desc->type
                         ? (OPENCL_LIBSMM_AI(desc->m, desc->n, desc->k, sizeof(double)) * opencl_libsmm_dacc)
                         : (OPENCL_LIBSMM_AI(desc->m, desc->n, desc->k, sizeof(float)) * opencl_libsmm_sacc));
                const int size = (int)MIN(sizeof(entry->size) / sizeof(*entry->size), entry->nexec);
                int blocks;
                OPENCL_LIBSMM_ISORT(entry->size, size);
                blocks = entry->size[size >> 1];
                ...
                opencl_libsmm_write_smm_params(stderr, 1 /*only_key*/, desc, NULL /*config*/, NULL /*delim*/,
                                               NULL /*begin*/, NULL /*close*/);
                fprintf(stderr, "=");
                opencl_libsmm_write_smm_params(stderr, 1 /*only_key*/, desc, &dummy, NULL /*delim*/, NULL /*begin*/,
                                               NULL /*close*/);
                fprintf(stderr, " ss=%i geo=%.1f", blocks, geo);
                if (0 < est)
                  fprintf(stderr, " est=%.1f", est);
...
#else
  const int mn = m * n;
  assert((NULL != dev_trs_stack && NULL != dev_data && NULL != stream && 0 <= offset && 0 <= stack_size) ||
         0 == stack_size);
  if ((
    ...
    if (NULL == config) {
...
#if defined(__DBCSR_ACC)
      int routine_handle;
      c_dbcsr_timeset(LIBSMM_ACC_TRANSPOSE_ROUTINE_NAME_STRPTR, LIBSMM_ACC_TRANSPOSE_ROUTINE_NAME_LENPTR,
                      &routine_handle);
#endif
      if (0 < nchar && (int)sizeof(fname) > nchar) {
        ...
        if (CL_SUCCESS == result) {
          ...
          const int bm =
              (0 >= blockm ? (NULL == config ? /*default*/ m : /*LIBXSMM_CLMP(config->bm, 1, m)*/ m) : MIN(blockm, m));
          size_t wgsize_max;
          ...
          if (EXIT_SUCCESS == result) {
            ...
            if (0 <= /*<*/ nchar && (int)sizeof(buffer) > nchar) {
              nchar = LIBXSMM_SNPRINTF(build_params, sizeof(build_params), param_format, cmem, inplace, fname, m, n,
                                       (int)new_config.wgsize, tname);
            }
          }
          if ('\0' != *tname && 0 < nchar && (int)sizeof(build_params) > nchar) {
            result =
                c_dbcsr_acc_opencl_kernel(OPENCL_LIBSMM_SOURCE_TRANSPOSE, fname, build_params, buffer, NULL /*try*/,
              ...
              if (EXIT_SUCCESS == result) {
                ...
                if (wgsize_max < new_config.wgsize) {
                  ...
                  if (0 < nchar && (int)sizeof(build_params) > nchar) {
                    result = c_dbcsr_acc_opencl_kernel(OPENCL_LIBSMM_SOURCE_TRANSPOSE, fname, build_params, buffer,
                ...
                if (EXIT_SUCCESS == result) {
                  config = (opencl_libsmm_trans_t *)OPENCL_LIBSMM_REGISTER(&key, sizeof(key), sizeof(new_config),
                                                                           &new_config);
#if !defined(OPENCL_LIBSMM_DEBUG_TRANS)
                  if (2 <= c_dbcsr_acc_opencl_config.verbosity || 0 > c_dbcsr_acc_opencl_config.verbosity) {
                    ...
                    fprintf(stderr, "INFO ACC/OpenCL: TRANS-kernel ");
                    opencl_libsmm_write_trans_params(stderr, 0 /*only_key*/, &key, NULL /*config*/, NULL /*delim*/,
                                                     NULL /*begin*/, NULL /*close*/);
                    fprintf(stderr, "=");
                    opencl_libsmm_write_trans_params(stderr, 0 /*only_key*/, &key, config, NULL /*delim*/,
                                                     NULL /*begin*/, NULL /*close*/);
    ...
    if (EXIT_SUCCESS == result) {
      ...
      { /* OpenCL is thread-safe except for clSetKernelArg and launching such shared kernel */
        ...
        ACC_OPENCL_CHECK(clEnqueueNDRangeKernel(queue, config->kernel, 1 /*work_dim*/, NULL, &work_size,
          ...
          if (EXIT_SUCCESS == result) {
            ...
            if (size <= config->nexec) {
              ...
              if ((i + 1) == s1) { /* fill config->size with median */
                const int s2 = size >> 1;
                OPENCL_LIBSMM_ISORT(config->size, size);
                config->size[s1] = config->size[s2];
                if (0 == (1 & s1))
                  config->size[s1] = (config->size[s1] + config->size[s2 - 1]) >> 1;
              }
            } else
              config->size[config->nexec++] = stack_size;
#endif
            if (4 <= c_dbcsr_acc_opencl_config.verbosity || 0 > c_dbcsr_acc_opencl_config.verbosity) {
              ...
              LIBXSMM_STDIO_ACQUIRE();
              fprintf(stderr, "INFO ACC/OpenCL: TRANS-kernel ");
              opencl_libsmm_write_trans_params(stderr, 1 /*only_key*/, &key, NULL /*config*/, NULL /*delim*/,
                                               NULL /*begin*/, NULL /*close*/);
              fprintf(stderr, "=");
              opencl_libsmm_write_trans_params(stderr, 1 /*only_key*/, &key, config, NULL /*delim*/, NULL /*begin*/,
                                               NULL /*close*/);
      ...
      if (EXIT_SUCCESS == result) {
        ...
        if (0 != c_dbcsr_acc_opencl_config.verbosity) {
          fprintf(stderr,
                  ...
                  offset, stack_size,
                  dbcsr_type_real_8 == datatype ? "f64" : (dbcsr_type_real_4 == datatype ? "f32" : "unknown"), m, n,
...
c_dbcsr_acc_bool_t libsmm_acc_process_suitable(c_dbcsr_acc_bool_t def_mnk, libsmm_acc_data_t datatype, int stack_size,
  ...
  if ((0 == result) && (2 <= c_dbcsr_acc_opencl_config.verbosity || 0 > c_dbcsr_acc_opencl_config.verbosity)) {
    opencl_libsmm_smm_t dummy;
    opencl_libsmm_smmkey_t key;
    key.type = datatype;
    key.m = m_max;
    ...
    LIBXSMM_STDIO_ACQUIRE();
    fprintf(stderr, "INFO ACC/OpenCL: SMM-kernel ");
    opencl_libsmm_write_smm_params(stderr, 1 /*only_key*/, &key, NULL /*config*/, NULL /*delim*/, NULL /*begin*/,
                                   NULL /*close*/);
    fprintf(stderr, "=");
    opencl_libsmm_write_smm_params(stderr, 1 /*only_key*/, &key, &dummy, NULL /*delim*/, NULL /*begin*/,
                                   NULL /*close*/);
    fprintf(stderr, " ss=%i", stack_size);
#if defined(OPENCL_LIBSMM_SUITABLE)
    if (0 < hst && 0 < acc)
      fprintf(stderr, " hst=%.1f acc=%.1f GFLOPS/s", hst, acc);
#endif
...
int libsmm_acc_process(const int *host_param_stack, const int *dev_param_stack, int stack_size, int nparams,
                       ...
                       int m_max, int n_max, int k_max, int max_kernel_dim, c_dbcsr_acc_bool_t def_mnk, void *stream,
                       void *c_stream) {
  int result = EXIT_SUCCESS;
  ...
  assert(0 == stack_size || (NULL != host_param_stack && NULL != dev_param_stack && NULL != dev_a_data &&
                             NULL != dev_b_data && NULL != dev_c_data));
  ...
  if (0 != libsmm_acc_process_suitable(def_mnk, datatype, stack_size, m_max, n_max, k_max, max_kernel_dim)) {
    ...
    if (CL_SUCCESS == result) {
      ...
      { /* determine kernel-kind (mini-batch vs. mini) */
        const char *const env_s = getenv("OPENCL_LIBSMM_SMM_S");
        const int s = ((NULL == env_s || '\0' == *env_s) ? /*default*/ 128 : atoi(env_s));
        if (NULL == config) {
          kernel_idx = ((OPENCL_LIBSMM_DEFAULT_BS * s) <= stack_size ? 0 : 1);
        } else if (1 != config->bs) {
          kernel_idx = (((MAX(config->bs, 1) * s) <= stack_size || (0 < config->s && config->s <= stack_size)) ? 0 : 1);
        } else
          kernel_idx = 1;
      }
      if (NULL == config || NULL == config->kernel[kernel_idx]) {
        ...
        char fname[ACC_OPENCL_MAXSTRLEN];
        int cl_level_major,
            nchar = LIBXSMM_SNPRINTF(fname, sizeof(fname),
                                     /* kernel name are meant to be unambiguous (BLAS-typeprefix and kernelsize) */
...
#if defined(__DBCSR_ACC)
        int routine_handle;
        c_dbcsr_timeset(LIBSMM_ACC_PROCESS_ROUTINE_NAME_STRPTR, LIBSMM_ACC_PROCESS_ROUTINE_NAME_LENPTR,
                        &routine_handle);
#endif
        if (0 < nchar && (int)sizeof(fname) > nchar &&
            EXIT_SUCCESS == c_dbcsr_acc_opencl_device_level(active_device, &cl_level_major, NULL /*level_minor*/,
                                                            NULL /*cl_std*/, &device_type)) {
          ...
          if (NULL != tname) {
            ...
            if (EXIT_SUCCESS == result) {
              ...
              const char *const env_bk = getenv("OPENCL_LIBSMM_SMM_BK"), *const env_bs = getenv("OPENCL_LIBSMM_SMM_BS");
              const char *const env_ws = getenv("OPENCL_LIBSMM_SMM_WS"), *const env_wg = getenv("OPENCL_LIBSMM_SMM_WG");
              const char *const env_lu = getenv("OPENCL_LIBSMM_SMM_LU"), *const env_nz = getenv("OPENCL_LIBSMM_SMM_NZ");
              const char *const env_al = getenv("OPENCL_LIBSMM_SMM_AL"), *const env_tb = getenv("OPENCL_LIBSMM_SMM_TB");
              const char *const env_tc = getenv("OPENCL_LIBSMM_SMM_TC"), *const env_ap = getenv("OPENCL_LIBSMM_SMM_AP");
              ...
              const int blocks = ((NULL == env_bs || '\0' == *env_bs) ? 0 : atoi(env_bs));
              ...
              const int default_bk =
                  ((0x0bd5 != devid && 0x020a != devid) ? (0 == kernel_idx ? m_max : MIN(OPENCL_LIBSMM_VMIN, m_max))
                                                        : 1);
              ...
              new_config.bk = (0 >= blockk ? (NULL == config ? default_bk : LIBXSMM_CLMP(config->bk, 1, m_max))
                                           : MIN(blockk, m_max));
              new_config.ws =
                  (0 >= wgmin ? (0 == kernel_idx
                                     ? (NULL == config ? /*default*/ n_max : LIBXSMM_CLMP(config->ws, 1, n_max * m_max))
                                     : /*default*/ n_max)
                              : MIN(wgmin, n_max * m_max));
              new_config.bs =
                  (0 >= blocks
                       ? (0 == kernel_idx ? (NULL == config ? OPENCL_LIBSMM_DEFAULT_BS : MAX(config->bs, 1)) : 1)
                       : blocks);
              new_config.wg = LIBXSMM_CLMP(
                  (NULL == env_wg || '\0' == *env_wg) ? (NULL == config ? default_wg : config->wg) : atoi(env_wg), -2,
                  2);
              new_config.lu =
                  LIBXSMM_CLMP((NULL == env_lu || '\0' == *env_lu)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ 0 : config->lu) : /*default*/ 0)
                                   : atoi(env_lu),
                               -1, 2);
              new_config.nz =
                  LIBXSMM_CLMP((NULL == env_nz || '\0' == *env_nz)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ 0 : config->nz) : /*default*/ 0)
                                   : atoi(env_nz),
                               0, 1);
              new_config.al =
                  LIBXSMM_CLMP((NULL == env_al || '\0' == *env_al)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ 0 : config->al) : /*default*/ 0)
                                   : atoi(env_al),
                               0, 1);
              new_config.tb =
                  LIBXSMM_CLMP((NULL == env_tb || '\0' == *env_tb)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ 0 : config->tb) : /*default*/ 0)
                                   : atoi(env_tb),
                               0, 1);
              new_config.tc =
                  LIBXSMM_CLMP((NULL == env_tc || '\0' == *env_tc)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ 1 : config->tc) : /*default*/ 1)
                                   : atoi(env_tc),
                               0, 1);
              new_config.ap =
                  LIBXSMM_CLMP((NULL == env_ap || '\0' == *env_ap)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ 1 : config->ap) : /*default*/ 0)
                                   : atoi(env_ap),
                               0, 1);
              new_config.aa =
                  LIBXSMM_CLMP((NULL == env_aa || '\0' == *env_aa)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ default_aa : config->aa)
                                                      : /*default*/ default_aa)
                                   : atoi(env_aa),
                               0, 3);
              new_config.ab =
                  LIBXSMM_CLMP((NULL == env_ab || '\0' == *env_ab)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ default_ab : config->ab)
                                                      : /*default*/ default_ab)
                                   : atoi(env_ab),
                               0, 3);
              new_config.ac =
                  LIBXSMM_CLMP((NULL == env_ac || '\0' == *env_ac)
                                   ? (0 == kernel_idx ? (NULL == config ? /*default*/ default_ac : config->ac)
                                                      : /*default*/ default_ac)
                                   : atoi(env_ac),
                               0, 2);
              ...
              if (0 != new_config.wg) {
                ...
                unsigned int r = libxsmm_remainder((unsigned int)new_config.wgsize[kernel_idx], OPENCL_LIBSMM_VMIN,
                ...
                if (0 > new_config.wg) {
                  ...
                  if (EXIT_SUCCESS == c_dbcsr_acc_opencl_device_ext(active_device, &extension, 1)) {
                    ...
                    if (-1 == new_config.wg) { /* cover entire WG-size in sub-group size */
                      ...
                      for (; (i * sizeof(size_t)) < nbytes; ++i) {
                        r = libxsmm_remainder((unsigned int)new_config.wgsize[kernel_idx], (unsigned int)sizes[i],
                                              &limit, NULL /*remainder*/);
                        if (r <= s) {
                          s = r;
                          sgs = sizes[i];
                ...
                wgsize_prf = new_config.wgsize[kernel_idx];
              if (2 <= new_config.wg)
                wgsize_prf = LIBXSMM_UP2POT(wgsize_prf);
              if (wgsize_prf < (2 * new_config.wgsize[kernel_idx]))
                new_config.wgsize[kernel_idx] = wgsize_prf; /* limit */
              assert(1 <= new_config.bs && 0 < new_config.wgsize[kernel_idx] && 0 < wgsize_max && 0 < wgsize_prf);
              ...
              if (new_config.wgsize[kernel_idx] <= wgsize_max) { /* SMMs can be potentially handled by device */
                ...
                if (NULL == env_barrier || '0' != *env_barrier) {
                  barrier_expr = ((0 != std_c11 && (0 == c_dbcsr_acc_opencl_config.devinfo.intel_id ||
                ...
                if (NULL == env_atomics || '0' != *env_atomics) {
                  ...
                  if (NULL == env_atomics || '\0' == *env_atomics || 0 != isdigit(*env_atomics)) {
                    ...
                    if (CL_SUCCESS == clGetDeviceInfo(active_device,
                      ...
                      if (NULL != extensions[1] && 1 < new_config.bs && 1 == new_config.bn && new_config.bm >= m_max &&
                          0 == new_config.al &&
                          (0 == (m_max & 1) ||
                           (0 == c_dbcsr_acc_opencl_config.devinfo.intel_id /*&& cl_nonv*/)) /* TODO */
                      ...
                      atomic_exp = "atomic_add_global_cmpxchg(A, B)";
                      atomic_ops =
                          (dbcsr_type_real_4 == datatype ? "-DCMPXCHG=atomic_cmpxchg" : "-DCMPXCHG=atom_cmpxchg");
                    } else {
                ...
                nchar = LIBXSMM_SNPRINTF(
                    ...
                    c_dbcsr_acc_opencl_config.devinfo.intel_id, cmem, (int)new_config.wgsize[kernel_idx], (int)sgs,
                    fname, NULL == env_nrepeat ? 1 : atoi(env_nrepeat), new_config.lu, m_max, n_max, k_max,
                    new_config.bs, new_config.bm, new_config.bn, new_config.bk, tname, datatype,
                    ...
                    0 == new_config.aa
                        ? ""
                        : (1 == new_config.aa ? "-DSLM_A=1" : (2 == new_config.aa ? "-DSLM_A=2" : "-DREG_A")),
                    0 == new_config.ab
                        ? ""
                        : (1 == new_config.ab ? "-DSLM_B=1" : (2 == new_config.ab ? "-DSLM_B=2" : "-DREG_B")),
                    0 == new_config.ac ? "" : (1 == new_config.ac ? "-DSLM_C=1" : "-DSLM_C=2"), atomic_type, atomic_ops,
                    atomic_exp, atomic_expr2, barrier_expr);
                if (0 < nchar && (int)sizeof(build_params) > nchar) {
#if !defined(NDBGDEV)
            ...
            if (EXIT_SUCCESS == result) {
              ...
              const char *const cl_try = ((EXIT_SUCCESS == cl_try_ok && 0 != c_dbcsr_acc_opencl_config.devinfo.intel_id)
              ...
              if (NULL != env_kernel) {
                ...
                if (NULL != src_kernel) {
                  const long int size =
                      (EXIT_SUCCESS == fseek(src_kernel, 0 /*offset*/, SEEK_END) ? ftell(src_kernel) : 0);
                  ...
                  if (NULL != src) {
                    if ((size_t)size == fread(src, 1 /*sizeof(char)*/, size /*count*/, src_kernel)) {
                      src[size] = '\0';
                      result = c_dbcsr_acc_opencl_kernel(src, fname, build_params, buffer, cl_try, &cl_try_ok,
                                                         extensions, sizeof(extensions) / sizeof(*extensions),
                                                         new_config.kernel + kernel_idx);
                    } else
                      libxsmm_free(src);
                  }
                  fclose(src_kernel);
              ...
              if (NULL == new_config.kernel[kernel_idx]) {
                result = c_dbcsr_acc_opencl_kernel(OPENCL_LIBSMM_SOURCE_MULTIPLY, fname, build_params, buffer, cl_try,
                                                   &cl_try_ok, extensions, sizeof(extensions) / sizeof(*extensions),
                                                   new_config.kernel + kernel_idx);
              }
              if (EXIT_SUCCESS == result) {
                size_t wgsize_max_kernel = wgsize_max;
                result = c_dbcsr_acc_opencl_wgsize(active_device, new_config.kernel[kernel_idx], &wgsize_max_kernel,
                                                   NULL /*prefmult*/);
                if (EXIT_SUCCESS == result) {
                  ...
                  if (new_config.wgsize[kernel_idx] <= wgsize_max_kernel) {
                    if (NULL == config || NULL == config->kernel[kernel_idx]) {
                      config = (opencl_libsmm_smm_t *)OPENCL_LIBSMM_REGISTER(&key, sizeof(key), sizeof(new_config),
                                                                             &new_config);
                    }
                    if (NULL != config) {
#if !defined(OPENCL_LIBSMM_DEBUG_SMM)
                      if (2 <= c_dbcsr_acc_opencl_config.verbosity || 0 > c_dbcsr_acc_opencl_config.verbosity) {
                        ...
                        duration = libxsmm_timer_duration(start, libxsmm_timer_tick());
                        fprintf(stderr, "INFO ACC/OpenCL: SMM-kernel ");
                        opencl_libsmm_write_smm_params(stderr, 0 /*only_key*/, &key, NULL /*config*/, NULL /*delim*/,
                                                       NULL /*begin*/, NULL /*close*/);
                        fprintf(stderr, "=");
                        opencl_libsmm_write_smm_params(stderr, 0 /*only_key*/, &key, &new_config, NULL /*delim*/,
                                                       NULL /*begin*/, NULL /*close*/);
                        fprintf(stderr, " gen=%.1f ms\n", 1E3 * duration);
                        LIBXSMM_STDIO_RELEASE();
                      }
                    ...
                    if (0 != c_dbcsr_acc_opencl_config.verbosity) {
                      fprintf(stderr, "ERROR LIBSMM: tile-size causes too large WG-size (min(%u,%u) < %u)!\n",
                              (unsigned int)wgsize_max_kernel, (unsigned int)wgsize_max,
      ...
      assert(EXIT_SUCCESS != result || (1 <= config->bs && 1 <= config->wgsize[kernel_idx]));
      ...
      if (EXIT_SUCCESS == result) {
        cl_event event, *const perf_event =
                            ((opencl_libsmm_timer_host == opencl_libsmm_timer ||
                              (0 <= c_dbcsr_acc_opencl_config.verbosity && 4 >= c_dbcsr_acc_opencl_config.verbosity))
                                 ? NULL
                                 : &event);
        const int bs = (0 == kernel_idx ? config->bs : 1);
        ...
        if (CL_SUCCESS == clGetMemObjectInfo(*ACC_OPENCL_MEM(dev_a_data), CL_MEM_SIZE, sizeof(size_t), &asize, NULL) &&
          ...
          libxsmm_gemm_descriptor *const desc =
              libxsmm_gemm_descriptor_dinit(&blob, precision, m_max, n_max, k_max, m_max, k_max, m_max, alpha, beta,
        ...
        { /* OpenCL is thread-safe except for clSetKernelArg and launching such shared kernel */
          ...
          ACC_OPENCL_CHECK(
              clSetKernelArg(config->kernel[kernel_idx], 3, sizeof(cl_mem), ACC_OPENCL_MEM(dev_param_stack)),
          ...
          if (3 <= c_dbcsr_acc_opencl_config.verbosity || 0 > c_dbcsr_acc_opencl_config.verbosity) {
            ...
            if (EXIT_SUCCESS == result) {
              ...
              if (4 <= c_dbcsr_acc_opencl_config.verbosity || 0 > c_dbcsr_acc_opencl_config.verbosity) {
                ...
                LIBXSMM_STDIO_ACQUIRE();
                fprintf(stderr, "INFO ACC/OpenCL: SMM-kernel ");
                opencl_libsmm_write_smm_params(stderr, 1 /*only_key*/, &key, NULL /*config*/, NULL /*delim*/,
                                               NULL /*begin*/, NULL /*close*/);
                fprintf(stderr, "=");
                opencl_libsmm_write_smm_params(stderr, 1 /*only_key*/, &key, config, NULL /*delim*/, NULL /*begin*/,
                                               NULL /*close*/);
        ...
        if (EXIT_SUCCESS == result) {
          ...
          if (0 != c_dbcsr_acc_opencl_config.verbosity) {
            fprintf(stderr, "libsmm_acc_process(size=%i, type=%s, m=%i, n=%i, k=%i, max=%i, stream=%p)", stack_size,
                    dbcsr_type_real_8 == datatype ? "f64" : (dbcsr_type_real_4 == datatype ? "f32" : "unknown"), m_max,
                    n_max, k_max, max_kernel_dim, stream);
          }
          ...
          for (i = 0; i < ((size_t)stack_size * nparams); i += nparams) {
            ...
            libxsmm_matdiff(&diff, (libxsmm_datatype)precision, m_max, n_max, gold + ic, test + ic, &m_max /*ldref*/,
                            &m_max /*ldtst*/);
