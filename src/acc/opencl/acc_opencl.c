...
#if !defined(NDEBUG)
void c_dbcsr_acc_opencl_notify(const char /*errinfo*/[], const void * /*private_info*/, size_t /*cb*/,
                               void * /*user_data*/);
    ...
    if (NULL == disable || '0' == *disable) {
      ...
      if (EXIT_SUCCESS == result) {
        ...
        if (CL_SUCCESS == clGetPlatformIDs(0, NULL, &nplatforms) && 0 < nplatforms) {
          ACC_OPENCL_CHECK(
              clGetPlatformIDs(nplatforms <= ACC_OPENCL_DEVICES_MAXCOUNT ? nplatforms : ACC_OPENCL_DEVICES_MAXCOUNT,
                               platforms, 0),
      ...
      for (i = 0; i < nplatforms; ++i) {
        ...
        if (CL_SUCCESS == clGetDeviceIDs(platforms[i], type, 0, NULL, &ndevices) && 0 < ndevices) {
          ...
          if (EXIT_SUCCESS == result) {
            ...
            const cl_device_partition_property properties[] = {CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN,
                                                               CL_DEVICE_AFFINITY_DOMAIN_NUMA, /*terminator*/ 0};
#endif
            cl_uint j = 0, n = 0;
            for (; j < ndevices; ++j) {
#if defined(CL_VERSION_1_2)
...
#endif
              {
                ...
                if (EXIT_SUCCESS ==
                    clCreateSubDevices(devices[j], properties, n,
      ...
      if (EXIT_SUCCESS == result && 0 < c_dbcsr_acc_opencl_config.ndevices) {
        ...
        if (NULL != env_vendor && '\0' != *env_vendor) {
          for (i = 0; i < (cl_uint)c_dbcsr_acc_opencl_config.ndevices;) {
            if (CL_SUCCESS == clGetDeviceInfo(c_dbcsr_acc_opencl_config.devices[i], CL_DEVICE_VENDOR,
      ...
      if (EXIT_SUCCESS == result && 0 < c_dbcsr_acc_opencl_config.ndevices) {
        ...
        if (NULL == env_device || '\0' == *env_device) {
          ...
          for (i = 0; i < (cl_uint)c_dbcsr_acc_opencl_config.ndevices; ++i) {
            ...
            result = clGetDeviceInfo(c_dbcsr_acc_opencl_config.devices[i], CL_DEVICE_TYPE, sizeof(cl_device_type),
                                     &itype, NULL);
            if (CL_SUCCESS == result) {
              if (0 != (CL_DEVICE_TYPE_DEFAULT & itype)) {
                ...
                break;
                         ...
                         device_id <= (int)i) {
                result = clGetDeviceInfo(c_dbcsr_acc_opencl_config.devices[i], CL_DEVICE_NAME, ACC_OPENCL_BUFFERSIZE,
                                         buffer, NULL);
      ...
      if (device_id < c_dbcsr_acc_opencl_config.ndevices) {
        if (EXIT_SUCCESS == result) {
          ...
          c_dbcsr_acc_opencl_config.contexts =
              (cl_context *)calloc(/* thread-specific */
        ...
        ACC_OPENCL_DEBUG_FPRINTF(
            stderr, "INFO ACC/OpenCL: started pid=%u nthreads=%i ndevices=%i (device=%i, context=%p).\n",
            libxsmm_get_pid(), c_dbcsr_acc_opencl_config.nthreads, c_dbcsr_acc_opencl_config.ndevices, device_id,
            NULL != c_dbcsr_acc_opencl_config.contexts
                ? ((const void *)c_dbcsr_acc_opencl_config.contexts[/*master*/ 0])
                : NULL);
      ...
      if (NULL != c_dbcsr_acc_opencl_config.stats) {
        ...
        for (i = 0; i < nelements; i += ACC_OPENCL_STREAMS_MAXCOUNT) {
          for (j = 0, nstreams = 0; j < ACC_OPENCL_STREAMS_MAXCOUNT; ++j) {
            if (NULL != c_dbcsr_acc_opencl_config.stats[i + j])
              ++nstreams;
          }
          if (0 != nstreams || 0 == i)
            fprintf(stderr, 0 < i ? " %i" : "%i", nstreams);
        }
...
int c_dbcsr_acc_opencl_device_vendor(cl_device_id device, const char vendor[]) {
  ...
  ACC_OPENCL_CHECK(clGetDeviceInfo(device, CL_DEVICE_VENDOR, ACC_OPENCL_BUFFERSIZE, buffer, NULL),
...
int c_dbcsr_acc_opencl_device_name(cl_device_id device, const char match[]) {
  ...
  ACC_OPENCL_CHECK(clGetDeviceInfo(device, CL_DEVICE_NAME, ACC_OPENCL_BUFFERSIZE, buffer, NULL), "retrieve device name",
                   result);
  if (EXIT_SUCCESS == result) {
...
int c_dbcsr_acc_opencl_device_level(cl_device_id device, int *level_major, int *level_minor, char cl_std[16],
                                    cl_device_type *type) {
  ...
  if (NULL != level_major || NULL != level_minor || NULL != cl_std) {
    ...
    if (CL_SUCCESS == result) {
      ...
      if (2 == sscanf(buffer, "OpenCL %u.%u", cl_std_level, cl_std_level + 1)) {
        if (NULL != level_major)
          *level_major = (int)cl_std_level[0];
        if (NULL != level_minor)
          *level_minor = (int)cl_std_level[1];
        if (NULL != cl_std) {
          if (2 <= cl_std_level[0]) {
...
int c_dbcsr_acc_opencl_device_ext(cl_device_id device, const char *const extnames[], int num_exts) {
  ...
  assert(NULL != device);
  ACC_OPENCL_CHECK(clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, ACC_OPENCL_BUFFERSIZE, extensions, NULL),
...
int c_dbcsr_acc_opencl_create_context(int thread_id, cl_device_id active_id) {
  ...
  if (CL_SUCCESS == result) {
    ...
    if (CL_SUCCESS != result && CL_INVALID_DEVICE != result) { /* retry */
      context =
          clCreateContext(NULL /*properties*/, 1 /*num_devices*/, &active_id, notify, NULL /* user_data*/, &result);
    }
    if (CL_SUCCESS == result) {
      assert(NULL != context);
          ...
          if (EXIT_SUCCESS == result) {
            if (NULL /*context*/ != inherit) {
              ...
              ACC_OPENCL_DEBUG_ELSE {
                ACC_OPENCL_DEBUG_FPRINTF(stderr, "ERROR ACC/OpenCL: releasing context %p (tid=%i) failed!\n",
                                         (const void *)inherit, thread_id);
              }
            }
        ...
        if (EXIT_SUCCESS == result) { /* update c_dbcsr_acc_opencl_config.devinfo */
#if defined(ACC_OPENCL_SVM)
          {
            ...
            c_dbcsr_acc_opencl_config.devinfo.svm_interop =
                (NULL == env_svm || 0 != atoi(env_svm)) &&
                (EXIT_SUCCESS == c_dbcsr_acc_opencl_device_level(active_id, &level_major, NULL /*level_minor*/,
                                                                 NULL /*cl_std*/, NULL /*type*/) &&
                 2 <= level_major);
          }
#endif
...
int c_dbcsr_acc_opencl_wgsize(cl_device_id device, cl_kernel kernel, size_t *max_value, size_t *preferred_multiple) {
  ...
  if (NULL != kernel) { /* kernel-specific */
    if (NULL != max_value) {
      ACC_OPENCL_CHECK(
          clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), max_value, NULL),
          "query maximum WG-size of kernel", result);
    }
    if (NULL != preferred_multiple) {
      ACC_OPENCL_CHECK(clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE,
    ...
    if (NULL != preferred_multiple) {
      ...
      ACC_OPENCL_CHECK(clGetDeviceInfo(device, CL_DEVICE_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, sizeof(size_t),
                                       preferred_multiple, NULL),
...
int c_dbcsr_acc_opencl_kernel(const char source[], const char kernel_name[], const char build_params[],
                              const char build_options[], const char try_build_options[], int *try_ok,
                              const char *const extnames[], int num_exts, cl_kernel *kernel) {
  ...
  if (EXIT_SUCCESS == result) {
    ...
    if (NULL != extnames) {
      ...
      for (; 0 < n; --n)
        if (NULL != extnames[n - 1]) {
          ...
          for (; NULL != ext;
               ext = ((ext + 1) < end ? strtok((ext + 1) + strlen(ext), ACC_OPENCL_DELIMS " \t") : NULL), ++nflat) {
            size_ext += strlen(ext);
          }
        }
      if (0 < size_ext && 0 < nflat) {
        ...
        char *const ext_source_buffer =
            (char *)libxsmm_aligned_scratch(size_src_ext + 1 /*terminator*/, 0 /*auto-align*/);
        if (NULL != ext_source_buffer) {
          for (n = 0; 0 < num_exts; --num_exts)
            if (NULL != extnames[num_exts - 1]) {
              ...
              char *ext = strtok(strncpy(buffer_name, extnames[num_exts - 1], ACC_OPENCL_MAXSTRLEN * 2 - 1),
                                 ACC_OPENCL_DELIMS " \t");
              for (; NULL != ext;
                   ext = ((ext + 1) < end ? strtok((ext + 1) + strlen(ext), ACC_OPENCL_DELIMS " \t") : NULL)) {
                const char *line = source;
                for (;;) {
                  if (2 != sscanf(line, "#pragma OPENCL EXTENSION %[^: ]%*[: ]%[^\n]", buffer,
                                  buffer + ACC_OPENCL_BUFFERSIZE / 2)) {
                    line = NULL;
                    break;
                ...
                if (EXIT_SUCCESS == system(buffer)) {
                  ...
                  if (0 < nchar && (int)sizeof(buffer) > nchar) {
                    ...
                    if (NULL != file) {
                      ...
                      char *const src =
                          (char *)(EXIT_SUCCESS == fseek(file, 0 /*offset*/, SEEK_SET)
    ...
    if (CL_SUCCESS == result) {
      int nchar = LIBXSMM_SNPRINTF(
          buffer, sizeof(buffer), "%s %s %s %s", cl_std, NULL != build_options ? build_options : "",
          NULL != try_build_options ? try_build_options : "", NULL != build_params ? build_params : "");
      assert(NULL != program);
      result =
          ((0 < nchar && (int)sizeof(buffer) > nchar)
        ...
        *kernel = clCreateKernel(program, kernel_name, &result);
        if (CL_SUCCESS == result) {
          ...
          if (2 <= c_dbcsr_acc_opencl_config.dump || 0 > c_dbcsr_acc_opencl_config.dump) {
            ...
            size_t size;
            binary = (unsigned char *)(CL_SUCCESS == clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t),
                                                                      &size, NULL)
        ...
        ACC_OPENCL_EXPECT(CL_SUCCESS, clGetProgramBuildInfo(program, active_id, CL_PROGRAM_BUILD_LOG,
                                                            ACC_OPENCL_BUFFERSIZE, buffer, NULL));
        ACC_OPENCL_EXPECT(CL_SUCCESS, clReleaseProgram(program));
      }
