...
int c_dbcsr_acc_stream_create(void **stream_p, const char *name, int priority) {
  ...
  if (CL_QUEUE_PRIORITY_HIGH_KHR <= priority && CL_QUEUE_PRIORITY_LOW_KHR >= priority) {
    ...
    if (0 != (1 & c_dbcsr_acc_opencl_config.priority) &&
        EXIT_SUCCESS == c_dbcsr_acc_stream_priority_range(&least, &greatest) && least != greatest) {
...
#pragma omp critical(c_dbcsr_acc_opencl_stream)
    ...
    LIBXSMM_ATOMIC_CMPSWP(c_dbcsr_acc_opencl_config.contexts + tid, NULL,
                          c_dbcsr_acc_opencl_config.contexts[/*master*/ 0], LIBXSMM_ATOMIC_RELAXED);
  } else
#endif
    tid = 0; /*master*/
  {
    ...
    if (NULL != context) {
      ...
      if (CL_SUCCESS == result) {
        const int s = (0 >= c_dbcsr_acc_opencl_config.share
                           ? 0
        ...
        if (1 >= s || s >= c_dbcsr_acc_opencl_config.nthreads || 0 == (tid % s)) {
          ...
          for (i = 0; n < c_dbcsr_acc_opencl_config.nthreads; n += s) {
            const int j = n + tid - 1,
                      t = j < c_dbcsr_acc_opencl_config.nthreads ? j : (j - c_dbcsr_acc_opencl_config.nthreads);
  ...
  if (EXIT_SUCCESS == result) {
    ...
    ACC_OPENCL_DEBUG_IF(EXIT_SUCCESS == result) {
      ACC_OPENCL_DEBUG_FPRINTF(stderr, "INFO ACC/OpenCL: create stream \"%s\" (tid=%i", NULL != name ? name : "unknown",
                               tid);
#if defined(ACC_OPENCL_STREAM_PRIORITIES)
      ACC_OPENCL_DEBUG_FPRINTF(stderr, ", priority=%i [%i->%i->%i]", priority, CL_QUEUE_PRIORITY_LOW_KHR,
...
int c_dbcsr_acc_stream_sync(void *stream) {
  ...
  const int *const priority =
      (0 == (1 & c_dbcsr_acc_opencl_config.flush) ? c_dbcsr_acc_opencl_stream_priority(stream) : NULL);
#endif
#if defined(__DBCSR_ACC) && defined(ACC_OPENCL_PROFILE)
  int routine_handle;
  static const char *const routine_name_ptr = LIBXSMM_FUNCNAME;
