...
int c_dbcsr_acc_host_mem_allocate(void **host_mem, size_t nbytes, void *stream) {
  ...
  if (CL_SUCCESS == result) {
    void *const mapped = clEnqueueMapBuffer(queue, memory, !c_dbcsr_acc_opencl_config.async, CL_MAP_READ | CL_MAP_WRITE,
...
int c_dbcsr_acc_dev_mem_allocate(void **dev_mem, size_t nbytes) {
  ...
  buffer = (
      ...
      0 != c_dbcsr_acc_opencl_config.devinfo.svm_interop
          ? clCreateBuffer(context, CL_MEM_USE_HOST_PTR, nbytes + ACC_OPENCL_OVERMALLOC,
                           clSVMAlloc(context, (cl_mem_flags)(CL_MEM_READ_WRITE | try_flag),
                                      nbytes + ACC_OPENCL_OVERMALLOC, 0 /*default alignment*/),
                           &result)
          :
#endif
          clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE | try_flag), nbytes + ACC_OPENCL_OVERMALLOC,
  ...
  if (0 != try_flag && CL_SUCCESS != result) { /* retry without try_flag */
    buffer = (
        ...
        0 != c_dbcsr_acc_opencl_config.devinfo.svm_interop
            ? clCreateBuffer(
                  context, CL_MEM_USE_HOST_PTR, nbytes + ACC_OPENCL_OVERMALLOC,
                  clSVMAlloc(context, CL_MEM_READ_WRITE, nbytes + ACC_OPENCL_OVERMALLOC, 0 /*default alignment*/),
                  &result)
            :
#endif
...
#if defined(ACC_OPENCL_SVM)
      void *const ptr =
          (0 != c_dbcsr_acc_opencl_config.devinfo.svm_interop ? c_dbcsr_acc_opencl_get_hostptr(buffer) : NULL);
#endif
...
int c_dbcsr_acc_memcpy_h2d(const void *host_mem, void *dev_mem, size_t nbytes, void *stream) {
  ...
  if (NULL != host_mem && NULL != dev_mem && 0 != nbytes) {
    ...
    result = clEnqueueWriteBuffer(*ACC_OPENCL_STREAM(stream), *ACC_OPENCL_MEM(dev_mem), CL_FALSE, 0 /*offset*/, nbytes,
    ...
    }
    {
      ...
      if (NULL == kernel) { /* generate kernel */
        const char source[] = "kernel void memset_zero(global uchar *restrict buffer) {\n"
                              ...
                              "  buffer[i] = pattern;\n"
                              "}\n";
        result = c_dbcsr_acc_opencl_kernel(source, "memset_zero" /*kernel_name*/, NULL /*build_params*/,
                                           NULL /*build_options*/, NULL /*try_build_options*/, NULL /*try_ok*/,
                                           NULL /*extnames*/, 0 /*num_exts*/, &kernel);
      }
      if (EXIT_SUCCESS == result) {
        assert(NULL != kernel);
        ACC_OPENCL_CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem), buffer), "set buffer argument of memset_zero kernel",
                         result);
        ACC_OPENCL_CHECK(clEnqueueNDRangeKernel(queue, kernel, 1 /*work_dim*/, &offset, &nbytes,
                                                NULL /*local_work_size*/, 0, NULL, NULL),
...
int c_dbcsr_acc_opencl_info_devmem(cl_device_id device, size_t *mem_free, size_t *mem_total, size_t *mem_local,
                                   int *mem_unified) {
...
int c_dbcsr_acc_dev_mem_info(size_t *mem_free, size_t *mem_total) {
  ...
  int result = 0 < c_dbcsr_acc_opencl_config.ndevices ? c_dbcsr_acc_opencl_device(ACC_OPENCL_OMP_TID(), &active_id)
                                                      : EXIT_FAILURE;
#if defined(__DBCSR_ACC) && defined(ACC_OPENCL_PROFILE)
  int routine_handle;
  static const char *const routine_name_ptr = LIBXSMM_FUNCNAME;
